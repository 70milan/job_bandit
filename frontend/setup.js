...
function initSession() {
    ...
    // New Modifiers: ESL and Short Responses
    const eslCheckbox = document.getElementById('setup-esl');
    if (eslCheckbox) {
        const savedEsl = localStorage.getItem('is_esl') === 'true';
        eslCheckbox.checked = savedEsl;
        eslCheckbox.addEventListener('change', () => {
            localStorage.setItem('is_esl', eslCheckbox.checked);
        });
    }

    const shortRespCheckbox = document.getElementById('setup-short-responses');
    if (shortRespCheckbox) {
        const savedShort = localStorage.getItem('short_responses') === 'true';
        shortRespCheckbox.checked = savedShort;
        shortRespCheckbox.addEventListener('change', () => {
            localStorage.setItem('short_responses', shortRespCheckbox.checked);
        });
    }
    ...
    if (convoBtn) {
        convoBtn.onclick = (e) => {
            ...
            if (window.ipcRenderer) {
                window.ipcRenderer.send('toggle-convo-window');
            }
        };
    }

    // Hotkeys button and modal (Setup Screen)
    ...
    const createHoverTooltip = (triggerId, contentHtml) => {
        ...
        let tooltip = document.getElementById(triggerId + '-tooltip');
        if (!tooltip) {
            tooltip = document.createElement('div');
            tooltip.id = triggerId + '-tooltip';
            tooltip.style.position = 'absolute';
            tooltip.style.display = 'none';
            tooltip.style.backgroundColor = 'rgba(30, 30, 30, 0.98)';
            tooltip.style.color = 'rgba(255, 255, 255, 0.85)';
            tooltip.style.border = '1px solid rgba(100, 255, 150, 0.3)';
        ...
        trigger.onmouseenter = (e) => {
            console.log(`[DEBUG_TOOLTIP] Mouse ENTERED trigger: ${triggerId}`);
    ...
    const hotkeysContent = '<div style="color: rgba(100,255,150,0.9); font-weight: bold; margin-bottom: 8px; text-transform: uppercase;">Keyboard Shortcuts</div>' +
        ...
        '<span style="color: rgba(100,255,150,0.8); font-weight: 600;">Ctrl+R</span><span style="color: #ccc;">Toggle system audio</span>' +
        '<span style="color: rgba(100,255,150,0.8); font-weight: 600;">Ctrl+M</span><span style="color: #ccc;">Toggle mic audio</span>' +
        '<span style="color: rgba(100,255,150,0.8); font-weight: 600;">Ctrl+K</span><span style="color: #ccc;">Screenshot + quick solve</span>' +
        '<span style="color: rgba(100,255,150,0.8); font-weight: 600;">Ctrl+S</span><span style="color: #ccc;">Screenshot + save context</span>' +
        '<span style="color: rgba(100,255,150,0.8); font-weight: 600;">Ctrl+Enter</span><span style="color: #ccc;">Process transcript with AI</span>' +
        '<span style="color: rgba(100,255,150,0.8); font-weight: 600;">Ctrl+Backspace</span><span style="color: #ccc;">Clear transcript</span>' +
        '<span style="color: rgba(100,255,150,0.8); font-weight: 600;">Ctrl+Q</span><span style="color: #ccc;">Toggle mini mode</span>' +
        ...
        '<span style="color: rgba(255,255,255,0.5);">Ctrl+Alt+Arrows</span><span style="color: #aaa;">Move app window</span>' +
        '<span style="color: rgba(255,255,255,0.5);">Ctrl+Alt+C</span><span style="color: #aaa;">Toggle convo window</span>' +
        '<span style="color: rgba(255,255,255,0.5);">Ctrl+Alt+Shift++</span><span style="color: #aaa;">Increase convo size</span>' +
        '<span style="color: rgba(255,255,255,0.5);">Ctrl+Alt+Shift+-</span><span style="color: #aaa;">Decrease convo size</span>' +
        '<span style="color: rgba(255,255,255,0.5);">Ctrl+Alt+Shift+O</span><span style="color: #aaa;">Reset convo size</span>' +
        '<span style="color: rgba(255,255,255,0.5);">Ctrl+Alt+Shift+Arrows</span><span style="color: #aaa;">Move convo window</span>' +
        '</div>';

    try {
        console.log("[DEBUG_TOOLTIP] Attaching tooltips...");
        createHoverTooltip('status-bar-hotkeys', hotkeysContent);
        // createHoverTooltip('btn-hotkeys-setup', hotkeysContent); // REMOVED: Should be a button on setup screen
        console.log("[DEBUG_TOOLTIP] Initialization finished.");
    } catch (err) {
        console.error("[DEBUG_TOOLTIP] Tooltip error: ", err);
    }
    ...
    try {
        ...
        if (resumeBtn) {
            resumeBtn.onclick = async () => {
                ...
                // Restore modifiers UI
                const eslCheckbox = document.getElementById('setup-esl');
                if (eslCheckbox) {
                    eslCheckbox.checked = !!sessionData.is_esl;
                    localStorage.setItem('is_esl', !!sessionData.is_esl);
                }
                const shortRespCheckbox = document.getElementById('setup-short-responses');
                if (shortRespCheckbox) {
                    shortRespCheckbox.checked = !!sessionData.short_responses;
                    localStorage.setItem('short_responses', !!sessionData.short_responses);
                }
...
async function handleCreateSession() {
    ...
    try {
        ...
        const sessionData = {
            ...
            is_esl: eslCheckbox ? eslCheckbox.checked : false,
            short_responses: shortRespCheckbox ? shortRespCheckbox.checked : false,
